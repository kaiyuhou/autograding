/* Allocate all memory to top of userspace (USERTOP), then deallocate 1 page using sbrk(-PGSIZE), then call shmem_access(1), then fork and have parent and child read and write. */
#include "types.h"
#include "stat.h"
#include "user.h"
...
main(int argc, char *argv[])
{
  ...
  if (pid < 0) {
    ...
    // make room for 1 shared page
    newSize = (int)sbrk(-PGSIZE);
    assert(newSize != -1);
